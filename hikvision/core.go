...
void fVoiceDataCallBack_cgo(LONG lVoiceComHandle, char *pRecvDataBuffer, DWORD dwBufSize, BYTE byAudioFlag, void* pUser);
...
// // 语音转发
func NET_DVR_StartVoiceCom_MR_V30(lUserId int, dwVoiceChan uint, fVoiceDataCallBack FVoiceDataCallBack, pUserData unsafe.Pointer) int {
	// return 1
	return int(C.NET_DVR_StartVoiceCom_MR_V30(
		C.LONG(lUserId),
		C.DWORD(dwVoiceChan),
		(C.FVoiceDataCallBack)(C.fVoiceDataCallBack_cgo),
		pUserData,
	))
}

//export fVoiceDataCallBackGo
func fVoiceDataCallBackGo(lUserID int, pRecvDataBuffer *C.char, dwBufSize uint, byAudioFlag byte, pUserData unsafe.Pointer) {

	pRecvDataBufferPtr := unsafe.Pointer(pRecvDataBuffer)
	fmt.Printf("%v", pRecvDataBufferPtr)

	if byAudioFlag == 0 {
		fmt.Printf("本地采集的数据 语音数据: %d \n", (*(*string)(pRecvDataBufferPtr)))
	} else if byAudioFlag == 1 {
		fmt.Printf("设备发送过来的 数据输入的数据%d \n", dwBufSize)
	} else if byAudioFlag == 2 {
		fmt.Printf("发送到设备 数据输入的数据%d \n", dwBufSize)
	} else {
		fmt.Println("未知数据")
	}
}
...
// 压缩信息
func NET_DVR_GetCurrentAudioCompress(lUserID int, info LPNET_DVR_COMPRESSION_AUDIO) bool {
	return goBOOL(C.NET_DVR_GetCurrentAudioCompress(
		C.LONG(lUserID),
		C.LPNET_DVR_COMPRESSION_AUDIO(unsafe.Pointer(&info)),
	))
}

// 初始化转码 初始化G711音频编码(Linux版本使用)。
func NET_DVR_InitG711Encoder(enc_info LPNET_DVR_AUDIOENC_INFO) C.LPVOID {
	return C.NET_DVR_InitG711Encoder(C.LPNET_DVR_AUDIOENC_INFO(unsafe.Pointer(enc_info)))
}

// 开启编码 G711音频编码(Linux版本使用)。
func NET_DVR_EncodeG711Frame(handle unsafe.Pointer, p_enc_proc_param LPNET_DVR_AUDIOENC_PROCESS_PARAM) bool {
	return goBOOL(C.NET_DVR_EncodeG711Frame(
		C.LPVOID(handle),
		C.LPNET_DVR_AUDIOENC_PROCESS_PARAM(unsafe.Pointer(p_enc_proc_param)),
	))
}

// 释放G711音频编码资源(Linux版本使用)。
func NET_DVR_ReleaseG711Encoder(pEncodeHandle unsafe.Pointer) bool {
	return goBOOL(C.NET_DVR_ReleaseG711Encoder(C.LPVOID(pEncodeHandle)))
}

// 转发语音数据。
func NET_DVR_VoiceComSendData(lVoiceComHandle int, pSendBuf []byte, dwBufSize uint) bool {
	return goBOOL(C.NET_DVR_VoiceComSendData(
		C.LONG(lVoiceComHandle),
		(*C.char)(unsafe.Pointer(&pSendBuf[0])),
		C.DWORD(dwBufSize),
	))
}

// 停止转发
func NET_DVR_StopVoiceCom(lVoiceComHandle int) bool {
	return goBOOL(C.NET_DVR_StopVoiceCom(C.LONG(lVoiceComHandle)))
}

func NET_DVR_InquiryRecordTimeSpan(lUserID int, dwChannel uint, lpInquiry LPNET_DVR_RECORD_TIME_SPAN_INQUIRY, lpResult LPNET_DVR_RECORD_TIME_SPAN) bool {
	return goBOOL(C.NET_DVR_InquiryRecordTimeSpan(
		C.LONG(lUserID),
		C.DWORD(dwChannel),
		C.LPNET_DVR_RECORD_TIME_SPAN_INQUIRY(unsafe.Pointer(lpInquiry)),
		C.LPNET_DVR_RECORD_TIME_SPAN(unsafe.Pointer(lpResult)),
	))
}

// 40
func NET_DVR_FindFile_V40(lUserId int, pFindCond LPNET_DVR_FILECOND_V40) int {
	return int(C.NET_DVR_FindFile_V40(
		C.LONG(lUserId),
		C.LPNET_DVR_FILECOND_V40(unsafe.Pointer(pFindCond)),
	))
}

func NET_DVR_FindFile_V30(lUserID int, pFindCond LPNET_DVR_FILECOND) int {
	return int(C.NET_DVR_FindFile_V30(
		C.LONG(lUserID),
		C.LPNET_DVR_FILECOND(unsafe.Pointer(pFindCond)),
	))
}

// 获取文件信息
func NET_DVR_FindNextFile_V40(lFindHandle int, lpFindData LPNET_DVR_FINDDATA_V40) int {
	return int(C.NET_DVR_FindNextFile_V40(
		C.LONG(lFindHandle),
		C.LPNET_DVR_FINDDATA_V40(unsafe.Pointer(lpFindData)),
	))
}

func NET_DVR_FindNextFile(lFindHandle int, lpFindData LPNET_DVR_FIND_DATA) int {
	return int(C.NET_DVR_FindNextFile(
		C.LONG(lFindHandle),
		C.LPNET_DVR_FIND_DATA(unsafe.Pointer(lpFindData)),
	))
}

// 关闭查找
func NET_DVR_FindClose_V30(lFindHandle int) bool {
	return goBOOL(C.NET_DVR_FindClose_V30(C.LONG(lFindHandle)))
}
