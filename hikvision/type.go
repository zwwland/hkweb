...
// AMR编码方式枚举。
type ENUM_AUDIO_ENC_MODE uint32

const (
	AMR_MR475_ ENUM_AUDIO_ENC_MODE = 0 //4.75 kb/s
	AMR_MR515_ ENUM_AUDIO_ENC_MODE = 1 //5.15 kb/s
	AMR_MR59_  ENUM_AUDIO_ENC_MODE = 2 //5.90 kb/s
	AMR_MR67_  ENUM_AUDIO_ENC_MODE = 3 //6.70 kb/s
	AMR_MR74_  ENUM_AUDIO_ENC_MODE = 4 //7.40 kb/s
	AMR_MR795_ ENUM_AUDIO_ENC_MODE = 5 //7.95 kb/s
	AMR_MR102_ ENUM_AUDIO_ENC_MODE = 6 //10.2 kb/s
	AMR_MR122_ ENUM_AUDIO_ENC_MODE = 7 //12.2 kb/s
	AMR_MRDTX_ ENUM_AUDIO_ENC_MODE = 8 //静音帧
)
...
// 音频数据回调函数
type FVoiceDataCallBack func(lVoiceComHandle int, pRecvDataBuffer *C.char, dwBufferSize uint, byAudioFlag byte, pUser unsafe.Pointer)

// 音频数据结构体
type NET_DVR_COMPRESSION_AUDIO struct {
	ByAudioEncType      byte // 0-G722; 1-G711_U; 2-G711_A; 5-MP2L2; 6-G726; 7-AAC; 8-PCM; 9-G722; 10-G723; 11-G729;
	ByAudioSamplingRate byte // 0-默认 1-16KHZ 2-32KHZ 3-48KHZ 4-44.1kHZ 5-8KHZ
	ByAudioBitRate      byte //
	Byres               [4]byte
	BySupport           byte //Mp2|2前四个字节的含义表示侯庙内容音数据的长度
}
type LPNET_DVR_COMPRESSION_AUDIO *NET_DVR_COMPRESSION_AUDIO

// 音频编码信息结构体(Linux版本使用)。
type NET_DVR_AUDIOENC_INFO struct {
	InFrameSize uint    // 输入一帧音频数据的大小(BYTES)：如果是G711编码，输入数据的大小为320字节；如果是G722编码，输入数据的大小为1280字节
	reserved    [16]int // 保留字节，置为0
}
type LPNET_DVR_AUDIOENC_INFO *NET_DVR_AUDIOENC_INFO

//音频编码数据结构体(Linux版本使用)。
type NET_DVR_AUDIOENC_PROCESS_PARAM struct {
	InBuf        *byte               //输入数据缓冲区，存放编码前PCM原始音频数据
	OutBuf       *byte               //输出数据缓冲区，存放编码后音频数据
	OutFrameSize uint16              //编码后一帧音频数据的大小(单位为字节)：如果是G711编码，输入数据的大小为160字节；如果是G722编码，输入数据的大小为80字节
	G726encReset int                 //G726重置开关(保留)
	G711Type     int                 //G711编码类型：0- U law，1- A law
	EncMode      ENUM_AUDIO_ENC_MODE //AMR音频编码模式配置(保留)
	Reserved     [16]int             //保留，置为0
}
type LPNET_DVR_AUDIOENC_PROCESS_PARAM *NET_DVR_AUDIOENC_PROCESS_PARAM

//通道录像起止时间查询条件结构体。
type NET_DVR_RECORD_TIME_SPAN_INQUIRY struct {
	DwSize uint16   // 结构体大小gg
	ByType byte     // 录像类型，0- 正常音视频录像，1- 图片通道录像，2- ANR通道录像，3- 抽帧通道录像
	ByRes  [63]byte //保留
}
type LPNET_DVR_RECORD_TIME_SPAN_INQUIRY *NET_DVR_RECORD_TIME_SPAN_INQUIRY

//通道录像起止时间查询结果结构体。
type NET_DVR_RECORD_TIME_SPAN struct {
	DwSize       uint16       //结构体大小
...
type LPNET_DVR_RECORD_TIME_SPAN *NET_DVR_RECORD_TIME_SPAN

//文件查找条件结构体
type NET_DVR_FILECOND_V40 struct {
	LChannel              int    //通道号
	DwFileType            uint16 //录象文件类型
	DwIsLocked            uint16 //是否锁定：0-未锁定文件，1-锁定文件，0xff表示所有文件（包括锁定和未锁定）
	DwUseCardNo           uint16 //是否带ATM信息进行查询：0-不带ATM信息，1-按交易卡号查询，2-按交易类型查询，3-按交易金额查询，4-按卡号、交易类型及交易金额的组合查询，5-按课程名称查找（此时卡号表示课程名称）
	SCardNumber           [CARDNUM_LEN_OUT]byte
	StruStartTime         NET_DVR_TIME
	...
	DwVolumeNum           uint16
	ByWorkingDeviceGUID   [GUID_LEN]byte
	USpecialFindInfo      *[8]byte
	ByStreamType          byte
	ByAudioFile           byte
	ByRes2                [30]byte
}
type LPNET_DVR_FILECOND_V40 *NET_DVR_FILECOND_V40

//智能查找条件联合体。
// type NET_DVR_SPECIAL_FINDINFO_UNION [8]byte
type NET_DVR_SPECIAL_FINDINFO_UNION NET_DVR_ATMFINDINFO

// type NET_DVR_SPECIAL_FINDINFO_UNION struct {
// 	ByLength        [8]byte
// 	StruATMFindInfo NET_DVR_ATMFINDINFO
// }
// type LPNET_DVR_SPECIAL_FINDINFO_UNION *NET_DVR_SPECIAL_FINDINFO_UNION

//ATM文件查找条件结构体。
type NET_DVR_ATMFINDINFO struct {
	ByTransactionType  byte
	ByRes              [3]byte
	DwTransationAmount uint32
}
type LPNET_DVR_ATMFINDINFO *NET_DVR_ATMFINDINFO

//文件查找结果信息结构体。
type NET_DVR_FINDDATA_V40 struct {
	SFileName     [100]byte
	StruStartTime NET_DVR_TIME
	StruStopTime  NET_DVR_TIME
	DwFileSize    uint16
...
type LPNET_DVR_FINDDATA_V40 *NET_DVR_FINDDATA_V40
...
type NET_DVR_FILECOND struct {
	...
	DwIsLocked    uint16       //是否锁定 0-正常文件,1-锁定文件, 0xff表示所有文件
	DwUseCardNo   uint16       //是否使用卡号
	SCardNumber   [32]byte     //卡号
	StruStartTime NET_DVR_TIME //开始时间
	StruStopTime  NET_DVR_TIME //结束时间
}
type LPNET_DVR_FILECOND *NET_DVR_FILECOND

// 查找的文件信息
type NET_DVR_FIND_DATA struct {
	SFileName     [100]byte    //文件名
	StruStartTime NET_DVR_TIME //文件的开始时间
	StruStopTime  NET_DVR_TIME //文件的结束时间
	dwFileSize    uint16       //文件的大小
}
type LPNET_DVR_FIND_DATA *NET_DVR_FIND_DATA
